function R_Hier = createHierarchicalCoarseSpace(s,A)
...
R_Hier=spalloc(2*d*(d-1)*s.levels,np,8*np);
% for kk=1:levels;
for k=1:d-1
    for j=1:d-1
        i=j+(k-1)*(d);
        ...
        for kk=1:s.levels
            r1=zeros(np,1);
            r1(d1)=solveHierBnd(p(:,d1),h,p(:,crspnt),kk,d1,s);
            r2=zeros(np,1);
            r2(d1)=r1(d1);
            b2=zeros(np,1);
            frNodes=ind(i,:);
        ...
        for kk=1:s.levels
            r1=zeros(np,1);
            r1(d2)=solveHierBnd(p(:,d2),h,p(:,crspnt),kk,d2,s);
            r2=zeros(np,1);
            r2(d2)=r1(d2);
            b2=zeros(np,1);
            frNodes=ind(i,:);
