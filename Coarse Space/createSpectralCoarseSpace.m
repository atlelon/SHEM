function R_Spec = createSpectralCoarseSpace(s,A)
    ...
    R_Spec=spalloc(2*d*(d-1)*s.levels,np,8*np);
    switch s.meshType
        case 'regular'
            for k=1:d-1
                for j=1:d-1
                    ...
                    V = solveEigBnd(p(:,d1),h,p(:,crspnt),d1,s);
                    for kk=1:s.levels
                        r1=zeros(np,1);
                        r1(d1)=V(:,kk);
                        r2=zeros(np,1);
                    ...
                    V = solveEigBnd(p(:,d2),h,p(:,crspnt),d2,s);
                    for kk=1:s.levels
                        r1=zeros(np,1);
                        r1(d2)=V(:,kk);
                        r2=zeros(np,1);
